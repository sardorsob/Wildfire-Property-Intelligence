...
These website-facing JSON files are generated by the archived backend export scripts in `_archive/backend/`, especially `export_all.py`, `export_neighbor_divergence.py`, and `export_neighbor_divergence_pooled.py`. In other words, the expected frontend outputs are derived from the backend export pipeline rather than being edited manually.
